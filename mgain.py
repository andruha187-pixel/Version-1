    ...
    while True:
        try:
            async with websockets.connect(BINANCE_WS,ping_interval=20,ping_timeout=20,close_timeout=5,max_size=4_000_000) as ws:
                log.info('BINANCE V4.2 WS connected | %s', BINANCE_SYMBOL.upper())
                ws_started = time.monotonic()
                async for raw in ws:
                    if time.monotonic() - ws_started >= WS_MAX_CONNECTION_AGE_SEC:
                        log.info('BINANCE periodic reconnect | trades=%d | ticks=%d', len(binance_trades), len(binance_tick_prices))
                        break
            ...
            log.warning('BINANCE V4.2 WS reconnect: %s', e)
            await asyncio.sleep(1)
...
async def health(request):
    ...
    return web.json_response({
        "ok": True,
        "version": "4.2-binance-memory-fix",
        "symbol": SYMBOL,
...
async def main():
    ...
    session = aiohttp.ClientSession(headers={
        "User-Agent": "PowerwinnerInspiredStrategySimulator/4.2-binance-memory-fix",
    ...
    log.info(
        "Strategy Simulator V4.2 MEMORY FIX started | %d variants | %.1fs cycle | lot=%.1f",
        len(VARIANTS),
        DECISION_INTERVAL,
        ORDER_SIZE,
    )
