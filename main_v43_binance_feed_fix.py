...
# V3 OUT-OF-SAMPLE FILTERS
V3_CONF_MIN = float(os.getenv("V3_CONF_MIN", "60"))
V3_DIRECTION_CHANGES_MIN = int(os.getenv("V3_DIRECTION_CHANGES_MIN", "100"))
V3_PATH_EFFICIENCY_MAX = float(os.getenv("V3_PATH_EFFICIENCY_MAX", "0.25"))
V3_EXTENDED_DISTANCE_PCT = float(os.getenv("V3_EXTENDED_DISTANCE_PCT", "0.0015"))
V3_EXTREME_RET_1S = float(os.getenv("V3_EXTREME_RET_1S", "0.0008"))

# V4 OUT-OF-SAMPLE FINGERPRINT FILTERS
V4_CONF_MIN = float(os.getenv("V4_CONF_MIN", "60"))
V4_RET10_MIN = float(os.getenv("V4_RET10_MIN", "0.00010"))
V4_LARGE_DELTA_MAX_TRIGGER = float(os.getenv("V4_LARGE_DELTA_MAX_TRIGGER", "0.34"))
V4_FLOW10_MIN_WHEN_LARGE = float(os.getenv("V4_FLOW10_MIN_WHEN_LARGE", "0.47"))
V4_BOOK_MIN = float(os.getenv("V4_BOOK_MIN", "0.05"))

# Keep CONF60 comparable with V3 after fixing the book feed.
V4_CONF_USE_BOOK = os.getenv("V4_CONF_USE_BOOK", "0").strip() == "1"
...
binance_trades = deque(maxlen=12000)       # (ms, price, quote_usd, aggressor_sign)
binance_second_prices = deque(maxlen=300) # (second_ts, last_price)
binance_tick_prices = deque(maxlen=20000)
...
binance_ws_messages = 0
binance_bad_trade_messages = 0
...
def _confidence_from_features(outcome, poly_ask, f):
    ...
    book_weight = W_BOOK if V4_CONF_USE_BOOK else 0.0
    weighted=W_IMPULSE*impulse+W_FLOW*flow+book_weight*book+W_LARGE*large+W_TREND*trend+W_DISTANCE*dist+W_POLY_PRICE*poly_component
...
def shadow_mode_pass(mode, f):
    ...
    conf60 = fresh and f["confidence"] >= V4_CONF_MIN

    direction = 1.0 if str(f.get("_outcome", "")).lower() == "up" else -1.0
    d_ret10 = direction * f["ret_10s"]
    d_flow10 = direction * f["flow_10s"]
    d_large10 = direction * f["large_delta_10s"]
    d_book = direction * f["book_imbalance"]

    ret10_ok = d_ret10 >= V4_RET10_MIN
    large_flow_bad = (
        d_large10 > V4_LARGE_DELTA_MAX_TRIGGER
        and d_flow10 <= V4_FLOW10_MIN_WHEN_LARGE
    )

    if mode == "CONF60":
        return conf60
    if mode == "V4_RET10":
        return conf60 and ret10_ok
    if mode == "V4_LARGE_FLOW_GUARD":
        return conf60 and not large_flow_bad
    if mode == "V4_COMBO":
        return conf60 and ret10_ok and not large_flow_bad
    if mode == "V4_BOOK_CONFIRM":
        return conf60 and d_book >= V4_BOOK_MIN
    return False
...
def process_binance_shadow_filters(condition, variant, asset, outcome, signal_type, filled, avg, gross, fee, total, signal_ms):
    ...
    for mode in BINANCE_FILTER_MODES:
        ...
        if accepted:
            reason = (
                f"conf={f['confidence']:.1f};regime={f['regime']};"
                f"dirchg={f['direction_changes']};path={f['path_efficiency']:.3f};"
                f"dist={f['distance_from_start_pct']:.5f};r1s={f['ret_1s']:.5f}"
            )
        elif signal_type == 'PYRAMID' and asset not in sides:
            reason = "no_shadow_position"
        else:
            reason = (
                f"blocked;conf={f['confidence']:.1f};regime={f['regime']};"
                f"dirchg={f['direction_changes']};path={f['path_efficiency']:.3f};"
                f"dist={f['distance_from_start_pct']:.5f};r1s={f['ret_1s']:.5f}"
            )
...
async def binance_ws_loop():
    ...
    global binance_ws_messages, binance_bad_trade_messages
    ...
    while True:
        try:
            async with websockets.connect(
                ...
                log.info('BINANCE V4.3 WS connected | %s | %s', BINANCE_SYMBOL.upper(), BINANCE_WS)
                ws_started = time.monotonic()
                last_diag = 0.0
                ...
                async for raw in ws:
                    if time.monotonic() - ws_started >= WS_MAX_CONNECTION_AGE_SEC:
                        log.info(
                            'BINANCE periodic reconnect | messages=%d | trades=%d | ticks=%d | depth=%d/%d',
                            binance_ws_messages, len(binance_trades), len(binance_tick_prices),
                            len(binance_depth_bids), len(binance_depth_asks),
                        )
                        break

                    data = json.loads(raw)
                    payload = data.get('data', data)
                    stream = str(data.get('stream', '')).lower()
                    event = str(payload.get('e', ''))
                    recv_ms = now_ms()
                    binance_ws_messages += 1
                    binance_last_event_ms = recv_ms

                    if 'aggtrade' in stream or event == 'aggTrade':
                        ts = si(payload.get('T') or payload.get('E') or recv_ms)
                        price = sf(payload.get('p'), 0.0)
                        qty = sf(payload.get('q'), 0.0)

                        # Never poison indicators with empty/invalid trade payloads.
                        if ts <= 0 or price <= 0 or qty <= 0 or not math.isfinite(price) or not math.isfinite(qty):
                            binance_bad_trade_messages += 1
                            log.warning('BINANCE bad aggTrade | stream=%s | payload=%s', stream, str(payload)[:300])
                            continue
                    ...
                    elif 'depth' in stream or event == 'depthUpdate':
                    ...
                    # Diagnostic proves that PRICE data, not merely depth traffic, is alive.
                    mono = time.monotonic()
                    if mono - last_diag >= 30:
                        last_diag = mono
                        px = _current_binance_price()
                        trade_age = recv_ms - binance_last_trade_ms if binance_last_trade_ms else 999999
                        depth_age = recv_ms - binance_last_depth_ms if binance_last_depth_ms else 999999
                        log.info(
                            'BINANCE DATA | price=%s | ticks=%d | trades=%d | trade_age=%dms | depth=%d/%d | depth_age=%dms | bad=%d',
                            f'{px:.2f}' if px is not None and math.isfinite(px) else 'NONE',
                            len(binance_tick_prices), len(binance_trades), trade_age,
                            len(binance_depth_bids), len(binance_depth_asks), depth_age,
                            binance_bad_trade_messages,
                        )

                        if trade_age > 5000:
                            log.warning('BINANCE PRICE FEED STALE: no valid aggTrade for %d ms; reconnecting', trade_age)
                            break

        except Exception as e:
            log.warning('BINANCE V4.3 WS reconnect: %s', e)

        await asyncio.sleep(1)


# ============================================================
# MEMORY CLEANUP / RENDER STABILITY
# ============================================================
...
def cleanup_resolved_market_memory():
    ...
    with db() as conn:
        rows = conn.execute(
            "SELECT condition_id, up_asset, down_asset "
            "FROM discovered_markets WHERE resolved=1 AND end_ts < ?",
            (cutoff,),
        ).fetchall()
    ...
    for key in list(strategy_state.keys()):
        if key[0] in old_cids:
            strategy_state.pop(key, None)

    for key in list(shadow_accepted_sides.keys()):
        if key[0] in old_cids:
            shadow_accepted_sides.pop(key, None)

    keep_assets = set()
    for m in markets.values():
        ua = str(m.get('up_asset') or '')
        da = str(m.get('down_asset') or '')
        if ua:
            keep_assets.add(ua)
        if da:
            keep_assets.add(da)

    for asset in list(books.keys()):
...
async def memory_maintenance_loop():
    ...
    while True:
        try:
            ...
            if removed or (now_mono - last_mem_log >= MEMORY_LOG_INTERVAL):
                rss = current_rss_mb()
                rss_text = f'{rss:.1f} MB' if rss is not None else 'n/a'
                log.info(
                    'MEMORY | RSS=%s | removed_markets=%d | markets=%d | books=%d | '
                    'strategy_state=%d | shadow_state=%d | subscribed_assets=%d | '
                    'binance_trades=%d | binance_ticks=%d',
                    rss_text,
                    removed,
                    len(markets),
                    len(books),
                    len(strategy_state),
                    len(shadow_accepted_sides),
                    len(subscribed_assets),
                    len(binance_trades),
                    len(binance_tick_prices),
                )
                last_mem_log = now_mono
        except Exception:
...
def make_report(start_ts, end_ts):
    ...
    lines += [
        "",
        "BINANCE V4 FINGERPRINT SHADOW — TOP 20",
    ]

    for x in bsummary[:20]:
        lines.append(
    ...
    for row in bsummary:
        m = row["mode"]
        r = mode_rollup.setdefault(
            m, {"pnl": 0.0, "cost": 0.0, "trades": 0, "wins": 0, "losses": 0}
        )
        r["pnl"] += sf(row["pnl"])
        r["cost"] += sf(row["cost"])
        r["trades"] += si(row["paper_trades"])
        r["wins"] += si(row["winning_markets"])
        r["losses"] += si(row["losing_markets"])

    lines += ["", "V4 MODE ROLLUP"]
    for mode in BINANCE_FILTER_MODES:
        r = mode_rollup.get(
            mode, {"pnl": 0.0, "cost": 0.0, "trades": 0, "wins": 0, "losses": 0}
        )
        roi = (r["pnl"] / r["cost"] * 100.0) if r["cost"] > 0 else 0.0
        lines.append(
            f"{mode}: pnl=${r['pnl']:+.2f} | ROI={roi:+.2f}% | "
            f"W/L={r['wins']}/{r['losses']} | trades={r['trades']}"
        )
    ...
    with zipfile.ZipFile(path, "w", zipfile.ZIP_DEFLATED) as z:
        ...
        z.writestr("binance_v4_features.csv", csv_bytes(binance_features_rows))
...
async def health(request):
    ...
    return web.json_response({
        "ok": True,
        "version": "4.3-binance-trade-feed-fix",
        "symbol": SYMBOL,
        ...
        "binance_last_trade_ms": binance_last_trade_ms,
        "binance_last_depth_ms": binance_last_depth_ms,
        ...
        "binance_price": _current_binance_price(),
        "binance_ws_messages": binance_ws_messages,
        "binance_bad_trade_messages": binance_bad_trade_messages,
        ...
        "binance_book_imbalance": _book_imbalance(),
        "binance_depth_feed_ok": bool(binance_depth_bids and binance_depth_asks),
        "memory_rss_mb": current_rss_mb(),
        "strategy_state_size": len(strategy_state),
        "shadow_state_size": len(shadow_accepted_sides),
        "time_utc": utc_iso(),
    })
...
async def main():
    ...
    session = aiohttp.ClientSession(headers={
        "User-Agent": "PowerwinnerInspiredStrategySimulator/4.3-binance-trade-feed-fix",
    ...
    log.info(
        "Strategy Simulator V4.3 BINANCE FEED FIX started | %d variants | %.1fs cycle | lot=%.1f",
        len(VARIANTS),
        DECISION_INTERVAL,
        ORDER_SIZE,
    )
