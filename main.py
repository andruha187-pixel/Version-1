...
def shadow_mode_pass(mode, f):
    ...
    conf60 = fresh and f["confidence"] >= V3_CONF_MIN

    if mode == "CONF60":
        return conf60

    if mode == "V3_REGIME":
        return conf60 and f["regime"] != "MIXED"

    if mode == "V3_DIR100":
        return conf60 and f["direction_changes"] >= V3_DIRECTION_CHANGES_MIN

    if mode == "V3_REGIME_OR_DIR100":
        return conf60 and (
            f["regime"] != "MIXED"
            or f["direction_changes"] >= V3_DIRECTION_CHANGES_MIN
        )

    if mode == "V3_PATH025":
        return conf60 and f["path_efficiency"] <= V3_PATH_EFFICIENCY_MAX

    if mode == "V3_EXHAUST_GUARD":
        direction = 1.0 if str(f.get("_outcome", "")).lower() == "up" else -1.0
        extended = (
            direction * f["distance_from_start_pct"] >= V3_EXTENDED_DISTANCE_PCT
        )
        still_impulsing = (
            direction * f["ret_1s"] >= V3_EXTREME_RET_1S
        )
        return conf60 and not (extended and still_impulsing)
    ...
    while True:
        try:
            async with websockets.connect(BINANCE_WS,ping_interval=20,ping_timeout=20,close_timeout=5,max_size=10_000_000) as ws:
                log.info('BINANCE V3 WS connected | %s', BINANCE_SYMBOL.upper())
                async for raw in ws:
            ...
            log.warning('BINANCE V3 WS reconnect: %s', e)
            await asyncio.sleep(1)
...
def make_report(start_ts, end_ts):
    ...
    with zipfile.ZipFile(path, "w", zipfile.ZIP_DEFLATED) as z:
        ...
        z.writestr("binance_v3_features.csv", csv_bytes(binance_features_rows))
...
async def health(request):
    ...
    return web.json_response({
        "ok": True,
        "version": "3.0-binance-regime-research",
        "symbol": SYMBOL,
...
async def main():
    ...
    session = aiohttp.ClientSession(headers={
        "User-Agent": "PowerwinnerInspiredStrategySimulator/3.0-binance-regime-research",
