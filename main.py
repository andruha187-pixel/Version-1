...
async def discovery_loop():
    while True:
        try:
            ...
            # Narrow query around current time to avoid pulling the whole catalog.
            data = await get_json(
                f"{GAMMA_API}/markets",
                params={
                    "limit": 100,
            ...
            if isinstance(data, list):
                for raw in data:
                    ...
                    mapping = {str(outcomes[i]).lower(): str(tokens[i]) for i in range(2)}
                    ...
                    start_dt = (
                        ...
                        or parse_iso(raw.get("startDate"))
                    )
                    ...
                    # 5m market start is reliably end - 300 sec when event start
                    # metadata is missing or broad.
                    end_ts = int(end_dt.timestamp())
                    start_ts = int(start_dt.timestamp()) if start_dt else end_ts - 300

                    # If startDate is obviously much earlier than a 5m window,
                    # use end-300.
...
async def main():
    ...
    session = aiohttp.ClientSession(headers={
        "User-Agent": "PowerwinnerInspiredStrategySimulator/1.0",
